...
# Import the process_all_maps_gpu function from control_prepare.py
from control_prepare import process_all_maps_gpu, get_memory_usage
...
def main(
    ...
    # Set up configuration for process_all_maps_gpu
    config = {
        "blur_size": blur_size,
    ...
    # Process maps using the optimized GPU function from control_prepare.py
    print(f"Using GPU-accelerated processing from control_prepare.py (batch_size={gpu_batch_size}, n_jobs={n_jobs})...")
    print(f"Memory usage before processing: {get_memory_usage():.2f} MB")
    ...
    if debug:
        for i, key in enumerate(final_clickmaps.keys()):
            if i >= 5:  # Limit to 5 examples in debug mode
                break
            image_path = os.path.join(clickme_image_folder, key)
            if os.path.exists(image_path):
                image_data = Image.open(image_path)
                plt.figure(figsize=(15, 10))
                plt.suptitle(f"Image: {key} - Clicks: {click_counts.get(key, 'unknown')}")
                
                for idx in range(min(len(final_clickmaps[key]), 16)):
                    plt.subplot(4, 5, idx + 1)
                    plt.imshow(create_clickmap_func([final_clickmaps[key][idx]], image_shape))
                    plt.axis("off")
                    plt.title(f"Trial {idx+1}")
                
                # Show mean clickmap and original image
                plt.subplot(4, 5, 19)
                plt.imshow(all_clickmaps[i].mean(0))
                plt.axis('off')
                plt.title("Mean clickmap")
                
                plt.subplot(4, 5, 20)
                plt.imshow(np.asarray(image_data))
                plt.axis('off')
    ...
    # The process_all_maps_gpu returns a list of arrays, correlation code expects a flat list
    ...
    num_clickmaps = len(processed_clickmaps)
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(compute_correlation_batch)(
            batch_indices=batch,
            all_clickmaps=processed_clickmaps,
            metric=metric,
    ...
    click_len = len(processed_clickmaps)
    ...
    return final_clickmaps, all_correlations, null_correlations, all_clickmaps, click_counts
...
if __name__ == "__main__":
    ...
    blur_sigma = config.get("blur_sigma", np.sqrt(blur_size))
    ...
    filter_duplicates = config.get("filter_duplicates", False)
    ...
    # Process data
    final_clickmaps, all_correlations, null_correlations, all_clickmaps, click_counts = main(
        clickme_data=clickme_data,
        blur_sigma=blur_sigma,
    ...
    # Save the results with click counts
    np.savez(
    ...
    # Also save click counts separately
    click_counts_json = json.dumps(click_counts, indent=4)
    with open(os.path.join(output_dir, f"{config['experiment_name']}_ceiling_click_counts.json"), 'w') as f:
